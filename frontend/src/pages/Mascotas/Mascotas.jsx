            ...
            try {
              ...
              if (data.ok && data.data?.length > 0) {
                const proximos = data.data
                  .filter(rec => !rec.completado && new Date(rec.fecha_programada) >= new Date())
                  .sort((a, b) => new Date(a.fecha_programada) - new Date(b.fecha_programada));
                if (proximos.length > 0) {
                  const rec = proximos[0];
                  const dias = Math.ceil(
                    (new Date(rec.fecha_programada) - new Date()) / (1000 * 60 * 60 * 24)
                  );
                  recordatoriosMap[m.id] = { texto: `${rec.nombre} · ${rec.tipo}`, dias };
                }
              }
            } catch { /* silencioso */ }
          }));
              ...
              <div key={m.id} className="mas-reminder-row">
                ...
                <div className="mas-reminder-right">
                  <span className={`mas-days-badge ${m.recordatorio.dias <= 7 ? 'soon' : m.recordatorio.dias <= 45 ? 'normal' : 'months'}`}>
                    {m.recordatorio.dias <= 30
                      ? `${m.recordatorio.dias} días`
                      : `${Math.round(m.recordatorio.dias/30)} mes${Math.round(m.recordatorio.dias/30) > 1 ? 'es' : ''}`}
                  </span>
  ...
  return (
    <div className="mas-card" style={style}>
      <div className="mas-card-img">
        {fotoUrl
          ? <img src={fotoUrl} alt={nombre} onError={e => { e.target.style.display='none'; }} />
          : null
        }
        {!fotoUrl && (
